...
function ThemeCard({ theme }: { theme: ThemeWithCounts }) {
  return (
    ...
    >
      ...
      {theme.description && (
        <p className="mt-1 line-clamp-2 text-sm text-stone-600">{theme.description}</p>
      )}
  ...
  return (
    <div>
      <div className="mb-4 flex gap-1 border-b border-stone-200">
        <Link
          href="/"
          className={`px-4 py-2 text-sm font-medium ${!showFresh ? "border-b-2 border-stone-900" : "text-stone-500"}`}
        >
          議論中
        </Link>
        <Link
