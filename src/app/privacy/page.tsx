    ...
    <article className="mx-auto flex max-w-2xl flex-col gap-6">
      ...
      <section>
        ...
        <p className={P}>
          ...
          アカウント登録はなく、原則として氏名・メールアドレス・電話番号などの個人情報は収集しません。
          例外は、お問い合わせ時に任意でご記入いただく返信先(第2条)だけです。
        </p>
      </section>
      ...
      <section>
        <h2 className={H}>4. 外部サービスへの送信</h2>
        <p className={P}>
          本サービスは以下の外部サービス上で動作します。ページの表示や機能の利用にあたり、
          お使いの端末から各社へ次の情報が送信されます。
        </p>
        <ul className={UL}>
          <li>
            <strong>Vercel(ホスティング・アクセス計測)</strong> — IPアドレス・ブラウザの種類・
            閲覧ページ・日時など。ページ配信と、Cookieを使わないアクセス状況の把握(Vercel Web Analytics)に使用
          </li>
          <li>
            <strong>Cloudflare(Turnstile / bot対策)</strong> — IPアドレス・ブラウザの種類・操作情報など。
            テーマ提案フォームでのbot判定にのみ使用
          </li>
          <li>
            <strong>Neon(データベース)</strong> — 投票・投稿などサービス上のデータの保存
          </li>
        </ul>
        <p className={P}>広告配信は行っていません。</p>
      </section>
      ...
      <section>
        ...
        <p className={P}>
          ...
          運営者自身は氏名・生のIPアドレスを保存しないため、法令に基づく開示請求があっても、
          運営者が保有する範囲で開示できる発信者情報は原則として存在しません。
          ただし、ホスティング事業者(Vercel)がアクセスログとして通信記録を一定期間保持している場合があります。
          いずれの場合も、保有する情報の範囲で法令・裁判所の命令に従い対応します。
        </p>
      </section>
